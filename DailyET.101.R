...
ET.B.1=ET1.101+ET4.101+ET7.101+ET10.101
...
ET.B.2=ET2.101+ET5.101+ET8.101+ET10.101
ET.B.2[1]=NA # can't calculate ET the first day
plot(data.101$Date.101, ET.B.2,
     ylim=c(0, 12), type='h', col='darkslategray4', lwd=2,
...
ET.B.3=ET3.101+ET6.101+ET9.101+ET10.101
ET.B.3[1]=NA # can't calculate ET the first day
plot(data.101$Date.101, ET.B.3,
     ylim=c(0, 12), type='h', col='darkslategray4', lwd=2,
...
# Left side of the onion bed
# Total water content (mm) of the soil profile
wc.1=(sm1*sd1+sm4*(sd4-sd1)+sm7*(sd7-sd4)+sm10*(sd10-sd7))/100
...
# Right side of the onion bed
# total water content (mm) of the soil profile
wc.2=(sm2*sd2+sm5*(sd5-sd2)+sm8*(sd8-sd5)+sm10*(sd10-sd8))/100

# 1st derivative - slope of the SM curve
f1=c() # 1st derivative
f1[1]=NA
for (i in 2:n) {
  f1[i]=(wc.2[i+1]-wc.2[i-1])/w
}
plot(data.101$Date.101, f1, 
     type='o', pch=19, cex=0.8, lwd=2, col='darkgoldenrod', 
     main='1st derivative', xlab='', ylab='f1')
...
sensor2=data.frame(Date=data.101$Date.101, SM=wc.2, f1, f2) # data frame with the results for sensor1
...
# Right side of the onion bed
# total water content (mm) of the soil profile
wc.3=(sm3*sd3+sm6*(sd6-sd3)+sm9*(sd9-sd6)+sm10*(sd10-sd9))/100

# 1st derivative - slope of the SM curve
f1=c() # 1st derivative
f1[1]=NA
for (i in 2:n) {
  f1[i]=(wc.3[i+1]-wc.3[i-1])/w
}
plot(data.101$Date.101, f1, 
     type='o', pch=19, cex=0.8, lwd=2, col='darkgoldenrod', 
     main='1st derivative', xlab='', ylab='f1')
...
sensor3=data.frame(Date=data.101$Date.101, SM=wc.3, f1, f2) # data frame with the results for sensor1
...
plot(sensor3$Date, sensor3$ET,
     type='h', lwd=3, col='deepskyblue',
     main='ET from the right side of the bed', xlab='', ylab='ET (mm)',
     ylim=c(0,15))
ET.D.3=sensor3$ET
...
f1[1]=NA
for (i in 2:n) {
  f1[i]=(SM9[i+1]-SM9[i-1])/w
}
plot(data.101$Date.101, f1, 
     type='o', pch=19, cex=0.8, lwd=2, col='darkgoldenrod', 
     main='1st derivative - soil depth 9', xlab='', ylab='f1')
abline(h=0, lwd=2)
...
plot(data.101$Date.101, f2, 
     type='o', pch=19, cex=0.8, lwd=2, col='brown2', 
     main='2nd derivative - soil depth 9', xlab='', ylab='f2')
abline(h=0, lwd=2)
...
# soil depth 10
SM10=sm10*(sd10-sd9)/100 # soil moisture (mm)

# 1st derivative - slope of the SM curve
f1=c() # 1st derivative
f1[1]=NA
for (i in 2:n) {
  f1[i]=(SM10[i+1]-SM10[i-1])/w
}
plot(data.101$Date.101, f1, 
     type='o', pch=19, cex=0.8, lwd=2, col='darkgoldenrod', 
     main='1st derivative - soil depth 10', xlab='', ylab='f1')
abline(h=0, lwd=2)
...
plot(data.101$Date.101, f2, 
     type='o', pch=19, cex=0.8, lwd=2, col='brown2', 
     main='2nd derivative - soil depth 10', xlab='', ylab='f2')
abline(h=0, lwd=2)
...
plot(data.101$Date.101, ET.E.1, 
     type='h', lwd=2,
     main='ET from left side of the bed', xlab='', ylab='ET (mm)',
     ylim=c(0,8))
...
ET.E.2=sensor2$ET+
         ...
         sensor8$ET+
         sensor10$ET
plot(data.101$Date.101, ET.E.2, 
     type='h', lwd=2,
     main='ET for the right side of the bed', xlab='', ylab='ET (mm)',
     ylim=c(0,8))
...
ET.E.3=sensor3$ET+
       ...
       sensor9$ET+
       sensor10$ET
plot(data.101$Date.101, ET.E.3, 
     type='h', lwd=2,
     main='ET for the furrow', xlab='', ylab='ET (mm)',
     ylim=c(0,8))
...
method=rep(c('A','B','D','E'), each=n)
ET=c(ET.A.1, ET.B.1, ET.D.1, ET.E.1) # Evapotranspiration
...
method=rep(c('A','B','D','E'), each=n)
ET=c(ET.A.2, ET.B.2, ET.D.2, ET.E.2) # Evapotranspiration
comparison.2=data.frame(date=data.101$Date.101, method, ET)

# Box-plot for ET
ggplot(comparison.2, aes(x = method, y = ET, fill = date)) +
  geom_boxplot() +
  scale_fill_viridis(discrete = TRUE, alpha=0.6, option="A") +
  theme_ipsum() +
  theme(
...
# Furrow
method=rep(c('A','B','D','E'), each=n)
ET=c(ET.A.3, ET.B.3, ET.D.3, ET.E.3) # Evapotranspiration
comparison.3=data.frame(date=data.101$Date.101, method, ET)

# Box-plot for ET
ggplot(comparison.3, aes(x = method, y = ET, fill = date)) +
  geom_boxplot() +
  scale_fill_viridis(discrete = TRUE, alpha=0.6, option="A") +
  theme_ipsum() +
  theme(
